...
禁止使用git restore, git stash, git checkout, git worktree指令

1. 在编写任何代码前，先描述你的方法并等待批准

2. 如果我给出的需求模糊，请在编写代码前提出澄清问题

3. 完成任何代码编写后，列出边缘案例并建议复盖它们的测试用例
...
| Web research / docs lookup / fact-checking | `researcher` | "search", "research", "搜寻", "研究", "查文档", "查一下", "帮我搜", Context7 + web search |
...
- ** 测试驱动 **：可测试设计，单元测试复盖，后台执行单元测试时，设置最大超时时间 60s，避免任务卡死。
...
- 每次进入 Plan 模式时，**必须优先调用 MCP 工具 `mcp__sequential-thinking__sequentialthinking`** 做多步思考，而不是直接在消息里即兴给计划；

- 第一次调用建议参数：

- `thought`：1–2 句话说明「正在为本次任务描述制定执行计划」（任务描述即 `/prompts:plan` 后紧跟的文本）；
...
- 后续调用可根据需要增加/减少 `totalThoughts`（例如发现子问题更多时增加 2–4 步），直到计划足够细致且可实施，再将 `nextThoughtNeeded = false` 结束思考；

- 不直接输出工具内部完整推理过程，只用其结果整理结构化计划。

🧠 复杂度分级与思考步数（totalThoughts）
...
- 推荐 `totalThoughts ≈ 10`，必要时可增加到 12 左右。
...
- Plan 模式约束无法从系统层硬性强制，仍依赖 LLM 严格遵守本节规则及 `prompts/plan.md`；

- 为提高可控性：

- 要求在 frontmatter 中记录 `tool: mcp__sequential-thinking__sequentialthinking` 与 `total_thoughts`；

- 推荐通过脚本周期性检查 `plan/*.md` 是否满足该约定（例如 grep 检查 `tool:` 和 `total_thoughts:` 字段）；
...
- **sequential-thinking**：分解复杂问题为 6-10 步可执行计划，输入简要问题/目标/约束
