...
![image-20251217100048045](RabbitMQ.assets/image-20251217100048045.png)

### RabbitMQ基础架构

* **基础架构图**

![image-20240806102134889](RabbitMQ.assets/image-20240806102134889.png) 
...
![image-20231102194452610](RabbitMQ.assets/image-20231102194452610.png)



使用上面创建Docker容器时指定的默认用户名、密码登录：

![image-20231102194633997](RabbitMQ.assets/image-20231102194633997.png)



![image-20231102194746743](RabbitMQ.assets/image-20231102194746743.png)
...
![image-20240806084908636](RabbitMQ.assets/image-20240806084908636.png)



### 2 创建队列

![image-20240725175936170](RabbitMQ.assets/image-20240725175936170.png)

队列名称：fan.queue.simple

![image-20260811150125718](RabbitMQ.assets/image-20260811150125718.png)

### 3 整合SpringBoot

#### 3.1 创建SpringBoot工程

![image-20260811150522626](RabbitMQ.assets/image-20260811150522626.png)
...
![image-20260811150933092](RabbitMQ.assets/image-20260811150933092.png)
...
```java
package com.fan.rabbitmq_producer;
...
class RabbitmqProducerApplicationTests {
    ...
    public static final String ROUTING_KEY_SIMPLE = "fan.queue.simple";
...
![image-20260811152633972](RabbitMQ.assets/image-20260811152633972.png)

#### 3.3 消费端工程

##### ①创建module

![image-20260811151543985](RabbitMQ.assets/image-20260811151543985.png)
...
```java
package com.fan.rabbitmq_consumer;
...
```java
package com.fan.rabbitmq_consumer.listener;
...
public class MyMessageListener {
    ...
    @RabbitListener(queues = {"fan.queue.simple"})
...
监听方法不能直接运行，通过主启动类运行微服务。消费端取走消息之后，队列中就没有消息了：

![image-20260811173231161](RabbitMQ.assets/image-20260811173231161.png)

![image-20260811173201942](RabbitMQ.assets/image-20260811173201942.png)

## RabbitMQ工作模式

* RabbitMQ提供了**多种工作模式**：简单模式，work模式 ，Publish/Subscribe发布与订阅模式，Routing路由模式，Topics主题模式等

![image-20240806102313708](RabbitMQ.assets/image-20240806102313708.png)
...
![image-20240806084946234](RabbitMQ.assets/image-20240806084946234.png) 
...
```java
public static final String EXCHANGE_DIRECT = "";
public static final String ROUTING_KEY_WORK = "fan.queue.work";
...
![image-20260811173752349](RabbitMQ.assets/image-20260811173752349.png)

![image-20260811173817451](RabbitMQ.assets/image-20260811173817451.png)
...
public class MyMessageListener {
    ...
    @RabbitListener(queues = {"fan.queue.work"})
    public void processMessage(String messageContent, Message message, Channel channel) {
...
> Server Port:10000 Message Content:Hello fan 0
> Server Port:10000 Message Content:Hello fan 2
> Server Port:10000 Message Content:Hello fan 4
> Server Port:10000 Message Content:Hello fan 6
> Server Port:10000 Message Content:Hello fan 8

##### ②消费端B

> Server Port:20000 Message Content:Hello fan 1
> Server Port:20000 Message Content:Hello fan 3
> Server Port:20000 Message Content:Hello fan 5
> Server Port:20000 Message Content:Hello fan 7
> Server Port:20000 Message Content:Hello fan 9

### **2 订阅模式类型**

订阅模式示例图：

![image-20240806085107277](RabbitMQ.assets/image-20240806085107277.png) 
...
![image-20240806085123819](RabbitMQ.assets/image-20240806085123819.png) 
...
| 组件   | 组件名称                                   |
| ------ | ------------------------------------------ |
| 交换机 | fan.exchange.fanout                        |
| 队列   | fab.queue.fanout01<br />fab.queue.fanout02 |

##### 2 创建交换机

<span style="color:blue;">**注意**</span>：发布订阅模式要求交换机是Fanout类型

![image-20260812091009142](RabbitMQ.assets/image-20260812091009142.png)

##### 3 创建队列并绑定交换机

![image-20260812091357695](RabbitMQ.assets/image-20260812091357695.png)

![image-20260812091535848](RabbitMQ.assets/image-20260812091535848.png)

此时可以到交换机下查看绑定关系：

![image-20260812091744090](RabbitMQ.assets/image-20260812091744090.png)

同样的操作，把fan.queue.fanout02也绑定上

##### 4 生产者代码

```java
public static final String EXCHANGE_FANOUT = "fan.exchange.fanout";
...
public class MyMessageListener {
    ...
    @RabbitListener(queues = {"fan.queue.fanout01"})
    ...
    @RabbitListener(queues = {"fan.queue.fanout02"})
...
![image-20260812092213651](RabbitMQ.assets/image-20260812092213651.png)
...
![image-20240806085148987](RabbitMQ.assets/image-20240806085148987.png) 
...
| 组件   | 组件名称             |
| ------ | -------------------- |
| 交换机 | fan.exchange.direct  |
| 路由键 | fan.routing.key.good |
| 队列   | fan.queue.direct     |

##### 2 绑定

![image-20260812101618306](RabbitMQ.assets/image-20260812101618306.png)

![image-20260812101630894](RabbitMQ.assets/image-20260812101630894.png)

##### 3 生产者代码

```java
public static final String ROUTING_EXCHANGE_DIRECT = "fan.exchange.direct";

public static final String ROUTING_KEY_GOOD = "fan.routing.key.good";

@Test
public void testSendMessageRouting() {
    rabbitTemplate.convertAndSend(ROUTING_EXCHANGE_DIRECT, ROUTING_KEY_GOOD, "Hello routing ~");
}
```
...
```java
@RabbitListener(queues = {"fan.queue.direct"})
...
![image-20260812102114591](RabbitMQ.assets/image-20260812102114591.png)
...
![image-20240806085214905](RabbitMQ.assets/image-20240806085214905.png) 
...
| 组件   | 组件名称                               |
| ------ | -------------------------------------- |
| 交换机 | fan.exchange.topic                     |
| 路由键 | #.error<br />order.*<br />\*.\*        |
| 队列   | fan.queue.message<br />fan.queue.order |

##### 2 创建交换机

![image-20260812102712743](RabbitMQ.assets/image-20260812102712743.png)

![image-20260812103441690](RabbitMQ.assets/image-20260812103441690.png)

![image-20260812103501506](RabbitMQ.assets/image-20260812103501506.png)

##### 4 生产者代码

```java
public static final String EXCHANGE_TOPIC = "fan.exchange.topic";
...
public class MyMessageListener {
    ...
    @RabbitListener(queues = {"fan.queue.message"})
    public void processMessage01(String messageContent, Message message, Channel channel) {
        System.out.println("Queue Message:" + messageContent);
    }

    @RabbitListener(queues = {"fan.queue.order"})
    public void processMessage02(String messageContent, Message message, Channel channel) {
...
![image-20260812103825411](RabbitMQ.assets/image-20260812103825411.png)
...
![image-20240806085244893](RabbitMQ.assets/image-20240806085244893.png) 
...
![image-20240806085305207](RabbitMQ.assets/image-20240806085305207.png) 

#### **3、发布订阅模式 Publish/subscribe**

需要设置类型为fanout的交换机，并且交换机和队列进行绑定，当发送消息到交换机后，交换机会将消息发送到绑定的队列

![image-20240806085325073](RabbitMQ.assets/image-20240806085325073.png) 

#### **4、路由模式 Routing**

需要设置类型为direct的交换机，交换机和队列进行绑定，并且指定routing key，当发送消息到交换机后，交换机会根据routing key将消息发送到对应的队列

 ![image-20240806085354471](RabbitMQ.assets/image-20240806085354471.png)
...
需要设置类型为topic的交换机，交换机和队列进行绑定，并且指定通配符方式的routing key，当发送消息到交换机后，交换机会根据routing key将消息发送到对应的队列
 ...
 ![image-20240806085413115](RabbitMQ.assets/image-20240806085413115.png)
...
![image-20240806092424473](RabbitMQ.assets/image-20240806092424473.png)

* **故障情况1**：

![image-20240806092503472](RabbitMQ.assets/image-20240806092503472.png)

消息没有发送到消息队列上，后果：消费者拿不到消息，业务功能缺失，数据错误

* **故障情况2**：

![image-20240806092558221](RabbitMQ.assets/image-20240806092558221.png)

消息成功存入消息队列，但是消息队列服务器宕机了，原本保存在内存中的消息也丢失了，即使服务器重新启动，消息也找不回来了。后果：消费者拿不到消息，业务功能缺失，数据错误

* **故障情况3**：

![image-20240806092653865](RabbitMQ.assets/image-20240806092653865.png)
...
logging:
  level:
    com.fan.rabbitmq_producer.config.MQProducerAckConfig: info
```
...
```java
package com.fan.rabbitmq_producer.config;
...
public class RabbitMQTest {  
    ...
    public static final String EXCHANGE_DIRECT = "fan.exchange.direct";
...
![image-20260812142330508](RabbitMQ.assets/image-20260812142330508.png)

创建之后，可以在列表中看到：

![image-20260812142404754](RabbitMQ.assets/image-20260812142404754.png)

##### 2、创建非持久化队列

![image-20260812142538476](RabbitMQ.assets/image-20260812142538476.png)

创建之后，可以在列表中看到：

![image-20260812142559675](RabbitMQ.assets/image-20260812142559675.png)

##### 3、绑定

![image-20260812142730290](RabbitMQ.assets/image-20260812142730290.png)
...
![image-20260812145106339](RabbitMQ.assets/image-20260812145106339.png)
...
![image-20260812151055151](RabbitMQ.assets/image-20260812151055151.png)

![image-20260812151104093](RabbitMQ.assets/image-20260812151104093.png)
...
![image-20260812151219909](RabbitMQ.assets/image-20260812151219909.png)
...
public class MyMessageListener {
    ...
    public static final String EXCHANGE_DIRECT = "fan.exchange.direct";
    public static final String ROUTING_KEY = "fan.routing.key.good";
    public static final String QUEUE_NAME  = "fan.queue.direct";
    ...
    public void processMessage(String dataString, Message message, Channel channel) throws IOException {
        ...
        log.info(
                "收到消息：{}，deliveryTag={}，redelivered={}",
                dataString,
                deliveryTag
        );
        try {
            log.info("消息 ACK 成功，deliveryTag={}", deliveryTag);
            ...
            if (!redelivered) {
                log.info("第一次消费失败，重新放回队列");
                // 5、如果没有被投递过，那就重新放回队列，重新投递，再试一次
                channel.basicNack(deliveryTag, false, true);
            } else {
                log.info("消息已经重新投递过，拒绝消息，不再重新入队");
...
![未命名文件](RabbitMQ.assets/%E6%9C%AA%E5%91%BD%E5%90%8D%E6%96%87%E4%BB%B6.png)

#### 7 多啰嗦一句

消费端如果设定消息重新放回队列，Broker重新投递消息，那么消费端就可以再次消费消息，这是一种“重试”机制，这需要消费端代码支持“<span style="color:blue;font-weight:bold;">幂等性</span>”。

## 六、消费端限流

### 1 概述

![image-20240806094300945](RabbitMQ.assets/image-20240806094300945.png)
...
![image-20260812171525906](RabbitMQ.assets/image-20260812171525906.png)
...
![image-20260812171624009](RabbitMQ.assets/image-20260812171624009.png)
...
![image-20260812172013601](RabbitMQ.assets/image-20260812172013601.png)
...
![image-20240806094631068](RabbitMQ.assets/image-20240806094631068.png)
...
![image-20260812172402818](RabbitMQ.assets/image-20260812172402818.png)

设置绑定关系：

![image-20260812172502896](RabbitMQ.assets/image-20260812172502896.png)
...
![image-20260812172731795](RabbitMQ.assets/image-20260812172731795.png)
...
public void testSendMessageTTL() {  
    ...
    rabbitTemplate.convertAndSend(    
            EXCHANGE_DIRECT,     
            ROUTING_KEY,     
            "Hello fan", messagePostProcessor);    
}
```

##### 2、查看效果

这次我们是发送到普通队列上：20s后就会过期

![image-20260812173427724](RabbitMQ.assets/image-20260812173427724.png)
...
![image-20260812174359477](RabbitMQ.assets/image-20260812174359477.png)
...
![image-20260812174548053](RabbitMQ.assets/image-20260812174548053.png)
...
![image-20260813091253984](RabbitMQ.assets/image-20260813091253984.png)
...
![image-20260813103957091](RabbitMQ.assets/image-20260813103957091.png)

生产者发送20条消息之后，消费端死信队列接收到前10条消息：

![image-20260813103927753](RabbitMQ.assets/image-20260813103927753.png)
...
![image-20260813104424123](RabbitMQ.assets/image-20260813104424123.png)

因为没有消费端监听程序，所以消息未超时前滞留在队列中：

![image-20260813104537754](RabbitMQ.assets/image-20260813104537754.png)

消息超时后，进入死信队列：

![image-20260813104614758](RabbitMQ.assets/image-20260813104614758.png)
...
![image-20251217100731579](RabbitMQ.assets/image-20251217100731579.png)
...
![image-20251217101326993](RabbitMQ.assets/image-20251217101326993.png)
...
"Mounts": [
     ...
     {
         "Type": "volume",
         "Name": "f90a4f936705f338310e71d9f2163c73178ad6c4b37811f72994209ecbce9f87",
         "Source": "/var/lib/docker/volumes/f90a4f936705f338310e71d9f2163c73178ad6c4b37811f72994209ecbce9f87/_data",
...
![image-20231107180045135](RabbitMQ.assets/image-20231107180045135.png)
...
![image-20260813112250298](RabbitMQ.assets/image-20260813112250298.png)

确认点2：如果创建新交换机时可以在type中看到x-delayed-message选项，那就说明插件安装好了

![image-20260813112308915](RabbitMQ.assets/image-20260813112308915.png)
...
![image-20260813112704754](RabbitMQ.assets/image-20260813112704754.png)
...
![image-20260813112753047](RabbitMQ.assets/image-20260813112753047.png)
...
![image-20260813113626040](RabbitMQ.assets/image-20260813113626040.png)
...
![image-20251217093113569](RabbitMQ.assets/image-20251217093113569.png) 
...
![image-20251217093257831](RabbitMQ.assets/image-20251217093257831.png) 
